...
public class NumberOfVisibleNodes {
	...
	class NodeWrapper {
		...
		public NodeWrapper(Node _node, int _height) {
			node = _node;
			height = _height;
		}
	}
	  ...
	  // Add any helper functions you may need here
		...
		int height2(Node node) {
			...
			queue.offer(new NodeWrapper(node, 1));
			while (!queue.isEmpty()) {
				NodeWrapper nw = queue.poll();
				max = Math.max(max, nw.height);
				if(nw.node.left != null)
					queue.offer(new NodeWrapper(nw.node.left, nw.height+1));
				if(nw.node.right != null)
					queue.offer(new NodeWrapper(nw.node.right, nw.height+1));
			}
			return max;
		}
