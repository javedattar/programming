 ...
 * 
 * Example: Given an array {3,2,5,8,1,4} and window size 3 the max addition subarray is {2,5,8}
 * 
 * Approach I: This is a sliding window pattern problem where window is given. Traverse through input array and
 * calculate sum of m elements from current index. If the sum is greater than previous sum then update the output
 * array
 * 
 * n is no. of elements in an array m size of output array (window or number of elements to add) Time:
 * O(m)+O(n-m)*O(m)+O(m) so Big O is O(n-m)*O(m)
 * 
 * 
 * author: Javed Attar
 */
