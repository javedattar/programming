...
public class RottingOranges {
	...
	public static void main(String[] args) {
		...
		System.out.println("Rotting Oranges 2 grid >> " + Arrays.deepToString(grid) + " rots in ==> "
				+ rottingOranges.orangesRotting2(grid));
	...
	public int orangesRotting2(int[][] grid) {
		...
		int minutes = 0;
		int freshCount = 0;
		Queue<Integer> rottingQueue = new LinkedList<>();
		for (int i = 0; i < grid.length; i++) {
			for (int j = 0; j < grid[i].length; j++) {
				switch (grid[i][j]) {
				case 1:
					...
					break;
				case 2:
					rottingQueue.add(i * (grid[i].length) + (j + 1));
					break;
				}
				if (grid[i][j] == 1) {
		...
		int rottenOrangesSize = rottingQueue.size();
		while (!rottingQueue.isEmpty() && rottenOrangesSize > 0) {
			Integer ro = rottingQueue.poll();
			// find adjacent for x, y => [x,y-1], [x,y+1], [x-1, y], [x+1, y]
			// see if any of them are fresh oranges, reset them to rotten and add to queue

			// Left node
			int y = ro % (grid[0].length);
			int x = ro - y / (grid[0].length);

			if (y - 1 >= 0 && grid[x][y - 1] == 1) {
				rottingQueue.add(ro - 1);
				grid[x][y - 1] = 2;
				freshCount--;
			}
			// Right node
			...
			if (x - 1 >= 0 && grid[x - 1][y] == 1) {
				rottingQueue.add(ro - grid[0].length);
				grid[x - 1][y] = 2;
				freshCount--;
			}
			// Bottom Node
